const fs = require('fs');
let content = fs.readFileSync('src/pages/Dashboard_backup.jsx', 'utf8').replace(/\r\n/g, '\n');
...
// 4. Remote Load (the most vital fix!)
const oldEffectBad = `            setUserProgress(prev => {
...
const correctedEffect = `            setUserProgress(prev => {
              const reconciled = { ...prev };
              Object.keys(remoteProgress).forEach(unitId => {
                reconciled[unitId] = Math.max(prev[unitId] || 0, remoteProgress[unitId]);
              });
...
content = content.replace(oldEffectBad, correctedEffect);

// 5. Replace getUserProfile with authToken
content = content.replace(
...
console.log('Fixed backup applied to Dashboard.jsx');
